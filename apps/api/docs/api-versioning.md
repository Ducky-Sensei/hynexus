...
```typescript
// src/main-dev.ts
const config = new DocumentBuilder().setTitle("NestJS hynexus API").setDescription("Production-ready NestJS API").setVersion("2.0").addTag("v1/products", "Product management endpoints (v1)").addTag("v2/products", "Product management endpoints (v2 - Latest)").build();
```
