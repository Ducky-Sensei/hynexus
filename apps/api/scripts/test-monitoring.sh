...
if docker ps | grep -q "hynexus-prometheus"; then
...
if docker ps | grep -q "hynexus-grafana"; then
...
TARGETS=$(curl -s http://localhost:9090/api/v1/targets | grep -c "hynexus-service" || true)
if [ "$TARGETS" -gt 0 ]; then
    echo -e "${GREEN}✓ Prometheus is scraping hynexus-service${NC}"
else
